/*
 * Copyright (c) 2018 VeriSilicon. All Rights Reserved.
...
static void k_data_char(char new_char)
{
    if (k_data_escape) {
        ...
        os_data_char(ktrans(new_char));//bin_data_char
        k_data_escape = 0;
    } else {
...
/* --length; */ /* new length includes only data and block check to come */
    ...
    return ((u32) os_data_addr - (u32) bin_start_address);
}

int load_serial_bin_and_check(u32 offset)
{
    int size;

    printf("## Ready for binary (kermit) download to 0x%08X at %d bps...\n",
        offset, 115200);
