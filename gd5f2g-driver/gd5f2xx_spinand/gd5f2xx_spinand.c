...
#include "gd5f2xx_spinand.h"

//#define BUFSIZE (10 * 64 * 2048)
//#define CACHE_BUF 2112 equal page+oob size
//each block has 256k+16k 10*pagenum*pagesize;BUFSIZE is meaning block-size
#define BUFSIZE (10 * 64 * 4096)
//each page has 4k+256;CACHE_BUF is meaning page-size
#define CACHE_BUF 4352

//static int buf_mode = 1;
				    ...
				    struct mtd_oob_region *oobregion)
{
	...
	oobregion->offset = (section * 16) + 8;//(section * 16) + 1;
	oobregion->length = 6;

	//oobregion->offset = (section * 16) + 4;//(section * 16) + 1;
	//oobregion->length = 12;
				     ...
				     struct mtd_oob_region *oobregion)
{
	...
	oobregion->length = 8; // marker.

	//oobregion->offset = (section * 16);
	//oobregion->length = 2; // marker.
...
static int spinand_cmd(struct spi_device *spi, struct spinand_cmd *cmd)
{
	...
	/* 
	 * Note: For Winbond w25n1g, 
	 * the dummy byte is before address at most command.
	 * However, the 0x03 command, the dymmy byte is after address.
	 */
	if(cmd->dummy_addr) {
		spi_message_add_tail(&x[2], &message);
		spi_message_add_tail(&x[1], &message);
	...
	if (cmd->n_tx) {
		...
		if (cmd->cmd == CMD_READ_RDM4X ||
		    cmd->cmd == CMD_PROG_PAGE_CLRCACHE4X ||
		    cmd->cmd == CMD_PROG_PAGE4X)
		x[3].tx_nbits = 4;
#endif
...
static int spinand_read_id(struct spi_device *spi_nand, u8 *id)
{
	...
	cmd.cmd = CMD_READ_ID;//opcode 
	cmd.n_rx = 3;//revi byte num
	cmd.rx_buf = &nand_id[0];//recive argument
	
/*
	cmd.n_addr = 1;//addr num
//	cmd.addr = 0x0;//addr value
	sprintf(cmd.addr,"0x0");
	cmd.n_rx = 2;//revi byte num
	cmd.rx_buf = &nand_id[0];//recive argument
*/
...
static int spinand_set_buf_mode(struct spi_device *spi_nand, u8 mode)
{
	int retval;
	u8 otp = 0;
...
static int spinand_enable_qio(struct spi_device *spi_nand)
{
	...
	if ((otp & OTP_QIO_MASK) == OTP_QIO_MASK) {
		spi_nand->mode = SPI_RX_QUAD | SPI_TX_QUAD;
		return otp;
	}
	otp |= OTP_QIO_MASK;
	retval = spinand_set_otp(spi_nand, &otp);
...
//	u16 row;
	...
	cmd.cmd = CMD_READ;
	cmd.n_addr = 3;
	cmd.addr[0] = (u8)((row & 0xff000000) >> 24);//page is row address
	cmd.addr[1] = (u8)((row & 0xff0000) >> 16);//MSB & LSB first addr is addr[23]
	cmd.addr[2] = (u8)((row & 0xff00) >> 8);

	
	//cmd.n_addr = 3;
	//cmd.addr[1] = (u8)((row & 0xff00) >> 8); // Fixme: addr[0]??, offset ?
	//cmd.addr[2] = (u8)(row & 0x00ff);
//	cmd.n_dummy = 1;
//	cmd.dummy_addr = 1; // cmd + dummy + addr...
//	cmd.n_addr = 2;
//	cmd.addr[0] = (u8)((row & 0xff00) >> 8);
//	cmd.addr[1] = (u8)(row & 0x00ff);
...
/*
	if(buf_mode ==  0) {
		...
		cmd.n_dummy = 1;
		cmd.dummy_addr = 0; // cmd + addr + dummy ...
	}
*/
	cmd.n_addr = 2;
	cmd.addr[0] = (u8)((column & 0xff00) >> 8);  
 ...
 * 1、read data from nand to cache 2、read data from cache to spi master/system memory
 */
 ...
 * @len:     the number of bytes to write  需要写多少字节的长度
 * @buf:     the buffer holding @len bytes  存放待写的@len个字节的缓存
 *
 * Description:
 ...
 */
 /*
 * 1、write enable:06h 2、write cache command:02h 3、write execute command:10h
 * @page_id :state->row  ----1block = 64pages,1 device = 1024block = 1024*64pages(2^16) 
 * @offset :state->colume -- 1page = (2k+128)byte(2^12)
 */
				...
				u32 page_id, u16 offset, u16 len, u8 *buf)
{
	...
	wbuf = devm_kzalloc(&spi_nand->dev, CACHE_BUF, GFP_KERNEL);//1 page size=(2k+128)
	if (!wbuf)
		return -ENOMEM;

	enable_read_hw_ecc = 1;
	retval = spinand_read_page(spi_nand, page_id, 0, CACHE_BUF, wbuf);//read 1 page to wbuf
	if (retval < 0) {
	...
	for (i = offset, j = 0; i < len; i++, j++)
		wbuf[i] &= buf[j];		//wbuf 为待发送给cache的数据，
		//buf里填充的是待写入的数据；offset 待写入数据的页内偏移字节长度
		//len 为待写入数据的字节长度。
		//summary：经过以上流程，wbuf[]里就填充的是已偏移后待写入的字节，前面先读，是为了保证未偏移部分的数据一致性。
...
*/
	...
	cmd.cmd = CMD_ERASE_BLK;
	cmd.n_addr = 3;
	//gd need to transfer 24bit addr
	cmd.addr[0] = (u8)((row & 0xff000000) >> 24);//page is row address
	cmd.addr[1] = (u8)((row & 0xff0000) >> 16);//MSB & LSB first addr is addr[23]
	cmd.addr[2] = (u8)((row & 0xff00) >> 8);
...
static int spinand_erase_block(struct spi_device *spi_nand, int block_id)
{
	...
	retval = spinand_erase_block_erase(spi_nand, block_id);//D8
	while (1) {
		retval = spinand_read_status(spi_nand, &status);//0F
		if (retval < 0) {
			dev_err(&spi_nand->dev,
				"error %d reading status register\n", retval);
			return retval;
...
//	printk("[zxf]%s, command = %d\n",__func__,command);
	...
	case NAND_CMD_READ0:
		state->buf_ptr = 0;
		/*read one page size = 4k+256*/
	//	spinand_read_page(info->spi, page, 0x0, 0x840, state->buf);
		spinand_read_page(info->spi, page, 0x0, 0x1100, state->buf);
		break;
	/* READOOB reads only the OOB because no ECC is performed. */
	case NAND_CMD_READOOB:
		state->buf_ptr = 0;
		/*read oob data offset is 2k = 0x800,obb datasize = 128byte-0x80*/
//		spinand_read_page(info->spi, page, 0x800, 0x80, state->buf);
		spinand_read_page(info->spi, page, 0x1000, 0x100, state->buf);
		break;
...
#ifdef CONFIG_MTD_SPINAND_QIO
	dev_info(&spi_nand->dev, "SPI-NAND in Quad, configure(B0h):0x%x\n",
		spinand_enable_qio(spi_nand));
#endif
...
MODULE_DESCRIPTION("SPI NAND driver for Giga Chip");
