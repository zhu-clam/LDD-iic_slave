...
/* delay x useconds */
void udelay(CK_UINT64 usec)
{
    timer_udelay(usec);
}
/* delay x mseconds */
void mdelay(CK_UINT32 msec)
{
    int i;
    for (i = 0; i < msec; i++) {
        timer_udelay(1000);
...
void CK_Console_Init()
{
    CK_Uart_Open(consoleuart, CK_Console_CallBack);
}

void CK_Console_Close() {
    CK_Uart_Close(consoleuart);
}

CK_INT32 reg_readbk32(CK_INT32 addr,CK_INT32 expe) {
    if (read_mreg32(addr) == expe)
        return 1;
    else
        return 0;
}
