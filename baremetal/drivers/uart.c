...
static CKStruct_UartInfo CK_Uart_Table[] =
{
  ...
  {3,CK_UART_ADDRBASE3,CK_UART3_IRQID,FALSE,NULL},
  {4,CK_UART_ADDRBASE4,CK_UART4_IRQID,FALSE,NULL},
};
...
/*
 * This function is used to set UART into Loopback mode.
 * Parameters:
 * uartid--a basepointer, could be one of UART0, UART1, UART2, UART3, or UART4.
 * return: NULL
*/
void CK_Uart_Set_Loopback_Mode(CK_Uart_Device uartid) {
    CKStruct_UartInfo *info;
    info = &(CK_Uart_Table[uartid]);
    // Enable Loopback
    info->addr[CK_UART_MCR] |= MCR_LOOPBACK;
}

/*
 * This function is used to unset UART of Loopback mode.
 * Parameters:
 * uartid--a basepointer, could be one of UART0, UART1, UART2, UART3, or UART4.
 * return: NULL
*/
void CK_Uart_Unset_Loopback_Mode(CK_Uart_Device uartid) {
    CKStruct_UartInfo *info;
    info = &(CK_Uart_Table[uartid]);
    // Enable Loopback
    info->addr[CK_UART_MCR] &= (~MCR_LOOPBACK);
...
/* This function is used to read from Transmit FIFO Read Register
 * in FIFO Access Mode.
 * Parameters:
 * 	 uartid-- a basepointer, could be one of UART0, UART1, UART2 or UART3.
 *   len   -- the number of bytes to read
 *   buf   -- the buffer used to store the results
 * return: SUCCESS or FAILURE
 */
CK_INT32 CK_Uart_Read_TFR(IN CK_Uart_Device uartid, IN CK_UINT32 len,
                            OUT CK_UINT8 *buf) {
    CKStruct_UartInfo *info;
    CK_UINT32 i = 0;
    CK_UINT32 timeout = 0;
    ...
    while (i < len) {
        if (info->addr[CK_UART_TFL] != 0) {
            *buf = info->addr[CK_UART_TFR];
            buf++;
            i++;
        } else if (timeout++ > 0x5000){
            CK_Uart_Close(consoleuart);
            CK_Console_Init();
            printf("\n\tread UART TFR timeout!\n");
            return FAILURE;
        }
    }
    return TRUE;
...
/* This function is used to enable SIR Mode
 * Parameters:
 * 	 uartid-- a basepointer, could be one of UART0, UART1, UART2 or UART3.
 */
CK_INT32 CK_UART_SIR_Enable(IN CK_Uart_Device uartid) {
    CKStruct_UartInfo *info = NULL;
    ...
    info->addr[CK_UART_MCR] |= MCR_SIRE;
    return TRUE;
}

/* This function is used to disable SIR Mode
 * Parameters:
 * 	 uartid-- a basepointer, could be one of UART0, UART1, UART2 or UART3.
 */
CK_INT32 CK_UART_SIR_Disable(IN CK_Uart_Device uartid) {
    CKStruct_UartInfo *info = NULL;
