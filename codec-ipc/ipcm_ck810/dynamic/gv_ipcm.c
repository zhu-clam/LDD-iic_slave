...
#define IPCM_POOL_START 0x40100000 //high 1G
...
//no need to memset to zero
	...
	if( size > 0 )
	{
		...
		ipcm_trace(TRACE_ZXF_DEBUG,"pMsg->msg_body->start:0x%x,pMsg->msg_body->size:%d",pMsg->msg_body->start,pMsg->msg_body->size);
	}
...
GV_IPCM_RetValue_E GV_IPCM_DestroyMessage(GV_IPCM_MSG_S *pstMsg)
{
	...
	{		
		...
		{
			ipcm_trace(TRACE_ZXF_DEBUG,"pstMsg->msg_alloc->size: %d",pstMsg->msg_alloc->size);
...
GV_IPCM_RetValue_E GV_IPCM_TryConnect(GV_IPCM_HANDLE_ATTR_S* stHandleAttr,GV_IPCM_MSG_POOL_S* ipcm_pool)
{
	...
	int ret = 0;
//	GV_IPCM_MSG_POOL_S* ipcm_pool = (GV_IPCM_MSG_POOL_S*)malloc(sizeof(*ipcm_pool));
	if (!ipcm_pool)
	{
		ipcm_trace(TRACE_ZXF_DEBUG,"malloc memory for ipcm message pool fail!");
		return -GV_ERR_MALLOC;
...
GV_IPCM_RetValue_E GV_IPCM_SendMessage(const GV_IPCM_MSG_S *pstMsg,GV_IPCM_MSG_POOL_S *msg_pool)
{
	...
	ipcm_trace(TRACE_ZXF_DEBUG,"entry send message begin bIsResp: %d,msg_len = %d",pstMsg->msg_body->bIsResp,msg_len);
	...
	ret = write(msg_pool->IpcmFd,(void *)pstMsg->msg_body,msg_len);//向共享内存写入整个消息
	if(ret != msg_len) {
...
//轮询从共享内存读消息，读取对端发送的物理地址和大小。如果物理地址为0 ，表示没有申请内存。
GV_IPCM_RetValue_E GV_IPCM_ReceiveMessage(GV_IPCM_MSG_POOL_S *msg_pool,GV_IPCM_MSG_S *pstMsgResp,GV_S32* msg_len)
{
	...
	if(!msg_pool)
	{
		ipcm_err("file desc is error 0x%x",msg_pool);
		return -GV_EVALID;
	}
	...
	ipcm_trace(TRACE_ZXF_DEBUG,"recv message after fd: %d",msg_pool->IpcmFd);	
	return ret;
}
/*
* 发送消息，经对端处理后，接收返回的结果！
* 实现：消息发送之后，轮询的判断是否设备节点可读，可以则将消息读取出来。完成！
*/
GV_IPCM_RetValue_E GV_IPCM_SendsyncMessage(GV_IPCM_MSG_POOL_S *msg_pool,const GV_IPCM_MSG_S *pstMsg,GV_IPCM_MSG_S *pstMsgResp)
{
	...
	ret = write(msg_pool->IpcmFd,pstMsg->msg_body,msg_len);// 将 pstMsg 发送到共享内存
	if(ret != msg_len) {
		ipcm_err("write error which return %d.",ret);
		ret = -GV_ERR_WRITE;
	}
...
/*轮询读取消息*/
	while(1) {
		...
		timeout.tv_usec = 0;
		FD_ZERO(&rfds);
		FD_SET(msg_pool->IpcmFd, &rfds);//判断设备文件是否可读
		ret = select(msg_pool->IpcmFd + 1, &rfds, NULL, NULL, &timeout);
		...
		if (FD_ISSET(msg_pool->IpcmFd, &rfds)) {
			len = read(msg_pool->IpcmFd,(void *)pstMsgResp->msg_body,MAX_SEND_LEN);//读取共享内存的消息
			if(len < 0) {
				ipcm_err("read error %d.",ret);
				ret = -GV_FAILED;
				break;
