...
* ck860 发送消息、ck810 打印
*/
void* ck860_message_pthread(void *arg)
{
...
/*for test ipcm connect */
	...
	GV_IPCM_MSG_S* pMsg = GV_IPCM_CreateMessage(fd_pool, 0, 2, 0);
	
	ret = GV_IPCM_SendMessage(pMsg,fd_pool);
	if (ret != GV_SUCCESS)
	{
...
//	ck860_play(&stConnectAttr);
	...
	if (0 != pthread_create(&audiopid, NULL, ck860_message_pthread, &stConnectAttr))
	{
